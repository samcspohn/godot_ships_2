...
void _ProjectileManager::_bind_methods() {
	...
	// Bind tracking methods
	ClassDB::bind_method(D_METHOD("track_damage_dealt", "p", "damage"), &_ProjectileManager::track_damage_dealt);
	ClassDB::bind_method(D_METHOD("track_damage_event", "p", "damage", "position", "type"),
						 &_ProjectileManager::track_damage_event);
	ClassDB::bind_method(D_METHOD("track_penetration", "p"), &_ProjectileManager::track_penetration);
	ClassDB::bind_method(D_METHOD("track_citadel", "p"), &_ProjectileManager::track_citadel);
...
// Tracking methods
void _ProjectileManager::track_damage_dealt(const Ref<ProjectileData> &p, double damage) {
	if (!p.is_valid()) {
		UtilityFunctions::print("track_damage_dealt: projectile is not valid");
		return;
	}

	Object *owner = p->get_owner();
	if (owner == nullptr) {
		UtilityFunctions::print("track_damage_dealt: owner is nullptr");
		return;
	}

	Variant stats_var = owner->get("stats");
	if (stats_var.get_type() == Variant::NIL) {
		UtilityFunctions::print("track_damage_dealt: stats is NIL");
		return;
	}

	Object *stats = Object::cast_to<Object>(stats_var);
	if (stats == nullptr) {
		UtilityFunctions::print("track_damage_dealt: stats object is nullptr");
		return;
	}
...
void _ProjectileManager::track_damage_event(const Ref<ProjectileData> &p, double damage,
										   const Vector3 &position, int type) {
...
void _ProjectileManager::track_penetration(const Ref<ProjectileData> &p) {
	if (!p.is_valid()) return;
...
void _ProjectileManager::track_citadel(const Ref<ProjectileData> &p) {
	if (!p.is_valid()) return;
...
void _ProjectileManager::track_overpenetration(const Ref<ProjectileData> &p) {
	if (!p.is_valid()) return;
...
void _ProjectileManager::track_shatter(const Ref<ProjectileData> &p) {
	if (!p.is_valid()) return;
...
void _ProjectileManager::track_ricochet(const Ref<ProjectileData> &p) {
	if (!p.is_valid()) return;
