...
func _init() -> void:
	...
	tooltip_stats = [
		{"stat": "HP Threshold",    "value": "< 25%"},
		{"stat": "Torpedo Reload",  "value": "-30%", "positive": true},
	]
...
func _a(ship: Ship) -> void:
	if ship.torpedo_controller == null:
		return
	if _active:
		(ship.torpedo_controller.params.dynamic_mod as TorpedoLauncherParams).reload_time *= 0.70
...
func _on_hp_changed(new_hp: float) -> void:
	...
	var should_be_active: bool = hp_ratio < 0.25
...
func apply(ship: Ship) -> void:
	_ship = ship
	_active = ship.health_controller._current_hp / ship.health_controller._max_hp < 0.25
