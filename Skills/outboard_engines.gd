...
func _init() -> void:
	...
	tooltip_stats = [
		{"stat": "Max Speed", "value": "+5% (above 75% HP)", "positive": true},
	]
...
func _on_hp_changed(new_hp: float) -> void:
	...
	var should_be_active: bool = hp_ratio > 0.75
...
func apply(ship: Ship) -> void:
	_ship = ship
	_active = ship.health_controller._current_hp / ship.health_controller._max_hp > 0.75
