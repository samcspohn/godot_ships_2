...
# Static list of upgrade paths - DirAccess.open() doesn't work in exported builds
# because res:// files are packed into a .pck file
# Add new upgrade scripts to this list when creating them
const UPGRADE_PATHS = [
...
# Load upgrades from the static list
func scan_upgrades():
...
func _load_upgrade(path: String):
	...
	if script:
		...
		push_error("UpgradeManager: Failed to load upgrade script: ", path)
...
func apply_upgrade_to_ship(ship: Ship, slot_index: int, upgrade_path: String):
	...
	if upgrade:
		...
		if ship.get_upgrades().upgrades[slot_index] != null:
			remove_upgrade_from_ship(ship, slot_index)
		ship.get_upgrades().add_upgrade(slot_index, upgrade)
		upgrade.apply(ship)
		return true
	return false
...
func remove_upgrade_from_ship(ship: Ship, slot_index: int):
	var upgrade = ship.get_upgrades().get_upgrade(slot_index)
	if upgrade:
		upgrade.remove(ship)
		ship.get_upgrades().remove_upgrade(slot_index)
		return true
	return false
