...
enum VALIDATION_MODE{
	SETUP,      # Load ship and prepare for validation
	RUN,        # Run the actual physics simulation
	STOPPED,
	OFF,
}

var val_mode = VALIDATION_MODE.OFF
var pending_shell_params: Dictionary = {}
...
func _physics_process(delta: float):
	...
	if val_mode == VALIDATION_MODE.SETUP:
		validation_setup()
		# After setup, wait one frame then run simulation
		val_mode = VALIDATION_MODE.RUN
	elif val_mode == VALIDATION_MODE.RUN:
		validation_run()
		val_mode = VALIDATION_MODE.OFF
	elif val_mode == VALIDATION_MODE.STOPPED:
		stop_validation()
		val_mode = VALIDATION_MODE.OFF
...
func _on_validate_pressed():
	if is_validating:
		# Stop validation - defer to physics thread
		val_mode = VALIDATION_MODE.STOPPED
	else:
		# Start validation - defer to physics thread (setup phase first)
		val_mode = VALIDATION_MODE.SETUP
...
func validation_setup():
	# Phase 1: Parse input, load ship, extract params
	# The actual simulation runs in validation_run() on the next physics frame
	...
	pending_shell_params = extract_shell_params()
	if pending_shell_params.is_empty():
	...
	validation_label.text = "Validation: Preparing simulation..."
	# val_mode will be set to RUN by _physics_process, allowing one frame for physics to sync

func validation_run():
	# Phase 2: Run the actual physics simulation
	# This runs one physics frame after setup, so colliders are registered

	if pending_shell_params.is_empty():
		validation_label.text = "Validation: No shell parameters"
		return

	# Run validation simulation
	run_validation_simulation(pending_shell_params)
	pending_shell_params = {}
