...
const VELOCITY_DECAY_BETA: float = 1.3
...
# func process_travel(projectile: ProjectileManager.ProjectileData, prev_pos: Vector3, t: float, space_state: PhysicsDirectSpaceState3D, events = []) -> ArmorResultData:
# 	var ray_query = PhysicsRayQueryParameters3D.new()
# 	ray_query.from = prev_pos
# 	ray_query.to = projectile.position
# 	ray_query.hit_back_faces = true
...
func process_hit(hit_node: ArmorPart, hit_position: Vector3, hit_normal: Vector3, projectile: ProjectileManager.ProjectileData, impact_velocity: Vector3, face_index: int, fuze: float, space_state: PhysicsDirectSpaceState3D, hit_water: bool, events) -> ArmorResultData:
	var first_hit_normal = hit_normal
	var armor_ray = PhysicsRayQueryParameters3D.new()
	armor_ray.hit_back_faces = true
	armor_ray.hit_from_inside = false
