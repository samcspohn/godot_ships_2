extends Node3D
class_name ThirdPersonSniperView
...
func handle_mouse_motion(event):
	...
	if target_lock_enabled:
		...
		rotation_degrees_horizontal -= event.relative.x * mouse_sensitivity * 0.6
		
		var cur_angle = atan2(-current_zoom / 2.0, sniper_range)
		print("DEBUG")
		print(cur_angle)
		print(tan(cur_angle))
		print(sniper_range)
		var delta = -event.relative.y * mouse_sensitivity * y_factor
		var next_angle = cur_angle + deg_to_rad(delta)
		rotation_degrees_vertical += delta
		next_angle = clamp(next_angle, -PI, -0.00001)
		if rotation_degrees_vertical > -45 and rotation_degrees_vertical < deg_to_rad(atan2(-current_zoom / 2.0, _ship.artillery_controller.guns[0].max_range)):
			rotation_degrees_vertical = cur_angle
		print(next_angle)
		print(tan(next_angle))
		# Fixed calculation using the correct inverse relationship
		sniper_range = -current_zoom / (2.0 * tan(next_angle))
		print(sniper_range)
		#sniper_range -= event.relative.y * mouse_sensitivity * y_factor
		sniper_range = clamp(sniper_range, -current_zoom / 2.0, _ship.artillery_controller.guns[0].max_range)
		#
...
func _update_camera_transform():
	...
	var cam_pos_sniper = ship_position + Vector3(
		0.0,
		sniper_range / 50.0 - ship_position.y + 15.0,
		0.0
	)
	...
	global_position = ship_position + cam_pos
	global_position.y = cam_pos.y
	if rotation_degrees_vertical < deg_to_rad(atan2(-current_zoom / 2.0, _ship.artillery_controller.guns[0].max_range)):
	...
	#
		...
		rotation.y = deg_to_rad(rotation_degrees_horizontal)
		rotation.x = deg_to_rad(rotation_degrees_vertical)
	_update_t()
