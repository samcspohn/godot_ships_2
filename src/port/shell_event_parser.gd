...
# Format: Processing Shell: caliber=460.0 type=1 speed=780.0 drag=1.8e-05 damage=14500.0
#         size=4.60 mass=1460.00 fire_buildup=0.0 fuze_delay=0.0350 pen_mod=1.0000
#         auto_bounce=60.0000 ricochet_angle=45.0000 overmatch=32 arming_threshold=76
static func parse_processing_shell_event(line: String) -> ShellEvent:
	var data = {}

	# Extract shell info (everything after "Processing Shell: ")
	var info_start = line.find("Processing Shell: ")
	if info_start == -1:
		return ShellEvent.new("Processing Shell", data)
	var raw_info = line.substr(info_start + len("Processing Shell: ")).strip_edges()
	data["shell_info"] = raw_info

	# Parse key=value pairs
	var kv_map := {}
	var tokens = raw_info.split(" ", false)
	for token in tokens:
		var eq_pos = token.find("=")
		if eq_pos > 0:
			var key = token.substr(0, eq_pos)
			var value = token.substr(eq_pos + 1)
			kv_map[key] = value

	# Map parsed keys to data dictionary with proper types
	# Floats
	var float_keys := {
		"caliber": "caliber",
		"speed": "speed",
		...
		"size": "size",
		"mass": "mass",
		"fire_buildup": "fire_buildup",
		"fuze_delay": "fuze_delay",
		"pen_mod": "pen_mod",
		"auto_bounce": "auto_bounce",
		"ricochet_angle": "ricochet_angle",
	}
	for log_key in float_keys:
		var data_key = float_keys[log_key]
		if kv_map.has(log_key):
			data[data_key] = float(kv_map[log_key])

	# Ints
	if kv_map.has("type"):
		data["shell_type"] = int(kv_map["type"])
	if kv_map.has("overmatch"):
		data["overmatch"] = int(kv_map["overmatch"])
	if kv_map.has("arming_threshold"):
		data["arming_threshold"] = int(kv_map["arming_threshold"])
