...
func _search(field: ReachField, team_id: int, id: int, key: int, opts: Dictionary) -> Dictionary:
	var sc: Dictionary = field.score_utility(team_id, id, key, opts)
	_escaping = bool(sc.get("escaping", false))
	return sc
...
func debug_text() -> String:
	if not _has_station:
		return "Utility: none"
	return "Utility%s %.2f%s | threat %.2f = %.1f units (%d shooters) reach %.1f reveal %.1f close %.2f risk %.0f | plan %.1f ms score %.1f ms" % [
		" (escaping)" if _escaping else "", _station_score, " shore" if _refined else "",
